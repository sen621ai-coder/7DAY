...
- Adds 171 player-facing T16-T19 definitions: six weapon families, two ammo
  ...
  defense families, seven fortress-building families, and reward materials.
- Adds 12 siege-support entities across T16-T19: disruptors, spotters and
  linesmen. The five-role squad remains bounded by the existing replacement
  and nearby-target caps.
- Server runtime handles reactive-wall mitigation, powered interception,
  bounded automatic repair, decoy diversion, armor-break hits, weapon heat,
  resonance injection, field repair, jamming and evacuation anchors.
- Source definitions are generated by Tools/generate_endgame_expansion.py and
  validated by Tools/validate_endgame_expansion.py.
...
- ENDGAME_EXPANSION_CATALOG.md is the implementation inventory with stable IDs,
  definition counts, development batches and acquisition routes.
- Runtime 1.18.0 implements both catalog batches; the documents remain the
  balancing and implementation reference.
