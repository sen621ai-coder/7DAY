...
namespace YFAutomation.CargoDrones
{
    ...
    public sealed class CargoHubConfiguration
    {
        ...
        public readonly CargoPosition? Entrance;
        ...
        public CargoHubConfiguration(Guid world,Guid hub,CargoPosition position,string owner)
            :this(world,hub,position,owner,0,new CargoBinding[0],null,null,false){}
        CargoHubConfiguration(Guid world,Guid hub,CargoPosition position,string owner,long revision,CargoBinding[] bindings,CargoBinding target,CargoPosition? entrance,bool paused)
        {
            if(world==Guid.Empty||hub==Guid.Empty||string.IsNullOrEmpty(owner))throw new ArgumentException("Invalid hub identity");
            WorldId=world;HubId=hub;Position=position;Owner=owner;Revision=revision;sources=(CargoBinding[])bindings.Clone();Target=target;Entrance=entrance;Paused=paused;
        }
        ...
        CargoHubConfiguration Copy(CargoBinding[] bindings,CargoBinding target,CargoPosition? entrance,bool paused)
        {return new CargoHubConfiguration(WorldId,HubId,Position,Owner,checked(Revision+1),bindings,target,entrance,paused);}
        public CargoHubConfiguration AddSource(string actor,long expectedRevision,CargoBinding source,CargoRules rules)
        {
            ...
            return Copy(sources.Concat(new[]{source}).ToArray(),Target,Entrance,Paused);
        }
        public CargoHubConfiguration RemoveSource(string actor,long expectedRevision,Guid source)
        {
            ...
            return updated.Length==sources.Length?this:Copy(updated,Target,Entrance,Paused);
        }
        ...
        public CargoHubConfiguration SetTarget(string actor,long expectedRevision,CargoBinding target,CargoRules rules)
        {
            ...
            return Target==null&&target==null||Target!=null&&Target.Matches(target)?this:Copy(sources,target,null,Paused);
        }
        // The entrance is the server-validated position of an owned beacon. The
        // native UI resolves that identity before this pure configuration model
        // stores it. It remains bound to the selected target and is cleared when
        // that target changes.
        public CargoHubConfiguration SetEntrance(string actor,long expectedRevision,CargoPosition? entrance,CargoRules rules)
        {
            CanEdit(actor,expectedRevision);if(rules==null)throw new ArgumentNullException("rules");
            if(entrance.HasValue&&(Target==null||entrance.Value.Y<1||entrance.Value.Y>250||!rules.CanDeliver(Position,entrance.Value)))throw new InvalidOperationException("入口航标必须在目标范围和有效高度内");
            return Entrance.Equals(entrance)?this:Copy(sources,Target,entrance,Paused);
        }
        public CargoHubConfiguration SetPaused(string actor,long expectedRevision,bool paused)
        {CanEdit(actor,expectedRevision);return paused==Paused?this:Copy(sources,Target,Entrance,paused);}
        internal static CargoHubConfiguration Restore(Guid world,Guid hub,CargoPosition position,string owner,long revision,CargoBinding[] sources,CargoBinding target,bool paused,CargoPosition? entrance=null)
        {
            ...
            if(entrance.HasValue)result=result.SetEntrance(owner,result.Revision,entrance,rules);
            return new CargoHubConfiguration(world,hub,position,owner,revision,sources,target,entrance,paused);
        }
    }
}
