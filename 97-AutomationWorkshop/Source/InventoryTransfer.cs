...
namespace YFAutomation
{
    public static class InventoryTransfer
    {
        ...
        public static int Move(ItemStack[] input,ItemStack[] output,Func<int,bool> inputLocked,Func<int,bool> outputLocked,Func<ItemValue,int> capacity)
        {
            if(input==null||output==null||output.Length<2)return 0;
            var sample=output[0];if(sample==null||sample.IsEmpty())return 0;
            for(int i=0;i<input.Length;i++)
            {
                var stack=input[i];if(stack==null||stack.IsEmpty()||inputLocked(i)||stack.itemValue.type!=sample.itemValue.type)continue;
