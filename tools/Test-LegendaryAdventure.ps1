...
# Relay hunts have their own catalog regression; do not count them as adventures.
Assert-Adventure ($quests.SelectNodes('//quest[not(starts-with(@id,"PZAECDefense")) and not(starts-with(@id,"PZAECRelayHunt"))]').Count -eq 104) 'Expected 40 mixed-type contracts, 60 affix contracts and four trials'
...
Assert-Adventure ($events.SelectNodes('//action_sequence[not(starts-with(@name,"PZAECDefense")) and not(starts-with(@name,"PZAECStronghold")) and not(starts-with(@name,"eventPZAECRelayHunt"))]').Count -eq 23) 'Original three + twenty adventure events required'
