...
public static class CargoDroneCoreTests
{
    ...
    public static string Run(string directory)
    {
        ...
        Check(CargoWarehouseFilter.RelativeLocation(new CargoPosition(10,100,20),new CargoPosition(7,104,24))=="距坪 6 格 · 北4格 西3格 上4格","target directions include offsets, elevation and three-dimensional distance");
        Check(CargoWarehouseFilter.RelativeLocation(hub,hub)=="距坪 0 格 · 原地 同层","same-position target direction is readable");
    ...
    static void ReturnChecks()
    {
        ...
        changed.Obstacles.Clear();for(int i=0;i<2000&&!motion.Arrived;i++)motion.Tick(100);
        Check(motion.Arrived&&motion.Position.Distance(home)<1e-7,"automatic retry resumes blocked return and preserves remaining corridor");
        changed=new ObstacleAirspace();motion=new CargoMotion(changed,home,new CargoPoint(40,100,0),600000,returnReserve:180000);
        for(int i=0;i<2000&&!motion.Arrived;i++)motion.Tick(100);
        changed.Obstacles.Add(new CargoBox(new CargoPoint(25,98,-2),new CargoPoint(27,102,2)));motion.ReturnHome();
        for(int i=0;i<200&&motion.Hold!=CargoHold.PathBlocked;i++)motion.Tick(100);
        held=motion.Position;int corridor=motion.ReturnWaypointCount;changed.Obstacles.Clear();motion.ReturnHome();motion.Tick(100);
        Check(motion.Position.Distance(held)>0&&motion.ReturnWaypointCount==corridor,"repeated recall immediately retries blocked edge without discarding return trail");
