...
public static class SakuraPreviewCompiler
{
    public static void Build(string[] sources, string[] references, string output)
    {
        ...
        var compilation = CSharpCompilation.Create("Sakura.Preview", trees, refs, options);
...
'@
$sources = Get-ChildItem (Join-Path $modRoot '96-SakuraPreview/Source') -Filter '*.cs' | Sort-Object Name | ForEach-Object FullName
$gameRefs = @(Get-ChildItem $managed -Filter '*.dll' | ForEach-Object FullName) + (Join-Path $modRoot '0_TFP_Harmony/0Harmony.dll')
[SakuraPreviewCompiler]::Build($sources, $gameRefs, (Join-Path $modRoot '96-SakuraPreview/Sakura.Preview.dll'))
