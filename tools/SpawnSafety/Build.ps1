...
public static class SpawnSafetyCompiler {
 public static void Build(string[] sources,string[] references,string output){
  var c=CSharpCompilation.Create("PZAEC.SpawnSafety",sources.Select(p=>CSharpSyntaxTree.ParseText(File.ReadAllText(p),path:p)),references.Select(p=>MetadataReference.CreateFromFile(p)),new CSharpCompilationOptions(OutputKind.DynamicallyLinkedLibrary).WithOptimizationLevel(OptimizationLevel.Release).WithDeterministic(true));
  using(var s=new MemoryStream()){var result=c.Emit(s);foreach(var d in result.Diagnostics.Where(d=>d.Severity>=DiagnosticSeverity.Warning))Console.WriteLine(d);if(!result.Success)throw new Exception("Spawn safety compile failed; installed DLL not changed");var temporary=output+".building";File.WriteAllBytes(temporary,s.ToArray());if(File.Exists(output))File.Replace(temporary,output,null);else File.Move(temporary,output);}
 }
}
'@
$sources=Get-ChildItem (Join-Path $root 'ZZZ-PZAEC_SpawnSafety/Source') -Filter '*.cs'|Sort-Object Name|ForEach-Object FullName
$refs=@(Get-ChildItem $managed -Filter '*.dll'|ForEach-Object FullName)+(Join-Path $root '0_TFP_Harmony/0Harmony.dll')
if(!$OutputPath){$OutputPath=Join-Path $root 'ZZZ-PZAEC_SpawnSafety/PZAEC.SpawnSafety.dll'}
[SpawnSafetyCompiler]::Build($sources,$refs,$OutputPath)
Write-Output 'Spawn safety compiled against installed V3.2 Mono/Unity references.'
