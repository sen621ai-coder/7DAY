...
public static class BasementLifecycleTests {
 ...
 public static void Run(){
  ...
  foreach(string field in new[]{"lamp","diffuser"})Check(typeof(PanelView).GetField(field,BindingFlags.NonPublic|BindingFlags.Instance).IsDefined(typeof(SerializeField),true),"Cloned runtime reference not serialized");
  var light=root.GetComponentsInChildren<Light>(true).Single();Check(!light.enabled,"Preview starts illuminated");
  var world=new WorldBase();view.Bind(world,new Vector3i());Call(view,"Update");Check(!light.enabled,"Unpowered lamp emits light");
  world.value.meta=2;world.tile.IsToggled=true;Time.time=1;Call(view,"Update");Check(light.enabled,"Powered and toggled lamp remains dark");
  world.tile.IsToggled=false;Time.time=2;Call(view,"Update");Check(!light.enabled,"Switch off ignored");
  world.tile.IsToggled=true;Time.time=3;Call(view,"Update");Call(view,"OnDisable");Check(!light.enabled && view.World==null,"Pooled lamp keeps stale world or light");
