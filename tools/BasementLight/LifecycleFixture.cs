...
public static class BasementLifecycleTests {
 ...
 public static void Run(){
  ...
  var lights=root.GetComponentsInChildren<Light>(true);Check(lights.Length==2,"Expected main and fill only");var light=lights.Single(l=>l.gameObject.name=="BasementSoftlight");var fill=lights.Single(l=>l.gameObject.name=="BasementWallCeilingFill");Check(fill.intensity==.35f && fill.range==14f && fill.shadowStrength==1,"Fill brightness/occlusion changed");Check(!light.enabled,"Preview starts illuminated");
  Check(light.intensity==1.2f && light.range==20f,"Runtime did not use revised brightness and range");
