...
Assert-Overflow ($guard::ClampValue([PassiveEffects]::RoundsPerMinute, [single]600, [single]-60) -eq [single]600) 'Invalid fire rate did not fall back to the weapon native rate.'
...
Assert-Overflow ($guard::ClampValue([PassiveEffects]::SpreadMultiplierAiming, [single]1, [single]-0.5) -eq [single]1) 'Invalid spread did not fall back to its native value.'
