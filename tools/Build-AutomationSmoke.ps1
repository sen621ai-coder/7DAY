...
public static class AutomationSmokeCompiler
{
...
'@
$sources = @((Join-Path $PSScriptRoot 'EscortNPC/AutomationSmoke.cs'))
$gameRefs = @(Get-ChildItem $managed -Filter '*.dll' | ForEach-Object FullName) + (Join-Path $modRoot '0_TFP_Harmony/0Harmony.dll')
if(-not $Output){$Output='E:/soft/7DTD-Modding/Automation020-Staging/AutomationSmoke.exe'}
$gameRefs += $RuntimeDll
[AutomationSmokeCompiler]::Build($sources, $gameRefs, $Output)
