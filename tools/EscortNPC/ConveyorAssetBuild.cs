  ...
  foreach(string kind in new[]{"Straight","Left","Right","Up","Down","Merge"}){
   ...
   if(kind=="Merge"){
    ...
    foreach(int side in new[]{-1,1}){
     Box(tr,"OutputRail",new Vector3(side*.39f,.25f,.43f),new Vector3(.085f,.14f,.14f),steel);
     for(int i=0;i<2;i++){var arrow=Box(tr,"InputArrow",new Vector3(side*.32f,.27f,(i==0?-1:1)*.05f),new Vector3(.16f,.015f,.035f),yellow);arrow.transform.localRotation=Quaternion.Euler(0,side*(i==0?45:-45),0);}
    }
   }else{
   for(int i=0;i<10;i++){
