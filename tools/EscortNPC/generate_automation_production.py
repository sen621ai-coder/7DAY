...
machines=[('yfAutoKitchen','自动厨房','FFAD80',[('yfAutoFrame',2),('yfAutoController',1),('yfAutoPump',1)],'首格样品选菜品，邻接输入箱放食材及烹饪锅/烤架等工具。沿用已解锁的篝火配方，所有者在线时生产。'),
 ...
 ('yfAutoRecycler','自动装备分解机','C299DB',[('yfAutoFrame',2),('yfAutoController',1),('yfAutoCutter',1)],'输入箱放待分解装备，输出首格放原版分解所得材料样品。一次分解一件，沿用原版分解回报。跳过六品质及以上、带模组、装有弹药或锁定格装备。'),
...
for id,title,tint,cost,desc in machines:
 ...
 recipe(id,cost)
 localize(id,title,desc+' 邻接通电的供电接口；输入紧贴机器，输出水平4米上下1米内；同一所有者、同一区块。输出首格样品保留，缺电/满箱暂停，进度随存档保存。','blocks')
